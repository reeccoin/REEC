...
{
    CDataStream serializedCoinSpend = ZREECModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZREECSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
...
bool UpdateZREECSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
