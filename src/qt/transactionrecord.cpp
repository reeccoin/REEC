        ...
        QList<TransactionRecord>& parts)
{
    ...
    if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zreecTracker->HasMintTx(hash))) {
        //zPIV stake reward
        sub.involvesWatchAddress = false;
        sub.type = TransactionRecord::StakeZREEC;
                                           ...
                                           QList<TransactionRecord>& parts)
{
    ...
    for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
        ...
        if (txout.IsZerocoinMint()) {
            ...
            sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
            sub.type = TransactionRecord::ZerocoinSpend_Change_zReec;
            sub.address = getValueOrReturnEmpty(wtx.mapValue, "zerocoinmint");
            if (!fFeeAssigned) {
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? ZREECModule::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
...
bool IsZREECType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZREEC:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zReec:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeZREEC ||
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeZREEC);
}
