...
{
    // Convert reec:// to reec:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("reec://"))
    {
        uri.replace(0, 12, "reec:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "REEC.lnk";
}
...
{
    return GetAutostartDir() / "reec.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("REEC-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  reec-qt [" + tr("command-line options") + "]                     " + "\n";
