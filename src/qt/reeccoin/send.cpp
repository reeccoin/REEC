...
#include "qt/reeccoin/forms/ui_send.h"
#include "qt/reeccoin/addnewcontactdialog.h"
#include "qt/reeccoin/qtutils.h"
#include "qt/reeccoin/sendchangeaddressdialog.h"
#include "qt/reeccoin/optionbutton.h"
#include "qt/reeccoin/sendconfirmdialog.h"
#include "qt/reeccoin/myaddressrow.h"
#include "qt/reeccoin/guitransactionsutils.h"
...
SendWidget::SendWidget(REECCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-reec");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination reecAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(reecAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(reecAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
