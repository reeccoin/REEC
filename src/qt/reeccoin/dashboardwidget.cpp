...
#include "qt/reeccoin/dashboardwidget.h"
#include "qt/reeccoin/forms/ui_dashboardwidget.h"
#include "qt/reeccoin/sendconfirmdialog.h"
#include "qt/reeccoin/txrow.h"
...
DashboardWidget::DashboardWidget(REECCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareReec, "square-chart-reec");
    setCssProperty(ui->labelSquarezReec, "square-chart-zreec");
    setCssProperty(ui->labelReec, "text-chart-reec");
    setCssProperty(ui->labelZreec, "text-chart-zreec");
    ui->labelAmountZreec->setVisible(false);
    ui->labelSquarezReec->setVisible(false);
    ui->labelZreec->setVisible(false);
    ...
    setCssProperty(ui->labelAmountReec, "text-stake-reec-disable");
    setCssProperty(ui->labelAmountZreec, "text-stake-zreec-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &REECCOINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &REECCOINGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &REECCOINGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isReec = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZREEC;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            reec = (pair.first != 0) ? pair.first / 100000000 : 0;
            zreec = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalReec += pair.first;
            chartData->totalZreec += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesReec.append(reec);
        chartData->valueszReec.append(zreec);

        int max = std::max(reec, zreec);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(15,91,153));
    set1->setColor(QColor(160,211,255));
    ...
    set0->append(chartData->valuesReec);
    set1->append(chartData->valueszReec);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalReec > 0 || chartData->totalZreec > 0) {
        setCssProperty(ui->labelAmountReec, "text-stake-reec");
        setCssProperty(ui->labelAmountZreec, "text-stake-zreec");
    } else {
        setCssProperty(ui->labelAmountReec, "text-stake-reec-disable");
        setCssProperty(ui->labelAmountZreec, "text-stake-zreec-disable");
    }
    forceUpdateStyle({ui->labelAmountReec, ui->labelAmountZreec});
    ui->labelAmountReec->setText(GUIUtil::formatBalance(chartData->totalReec, nDisplayUnit));
    ui->labelAmountZreec->setText(GUIUtil::formatBalance(chartData->totalZreec, nDisplayUnit, true));
