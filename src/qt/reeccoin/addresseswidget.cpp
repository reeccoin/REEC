...
#include "qt/reeccoin/forms/ui_addresseswidget.h"
#include "qt/reeccoin/addresslabelrow.h"
#include "qt/reeccoin/addnewaddressdialog.h"
#include "qt/reeccoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(REECCOINGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination reecAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(reecAdd, label.toUtf8().constData(),
