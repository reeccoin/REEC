...
#include "qt/reeccoin/forms/ui_snackbar.h"
#include "qt/reeccoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(REECCOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &REECCOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
