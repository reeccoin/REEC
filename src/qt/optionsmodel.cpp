...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::REEC);
    ...
    if (settings.contains("nAnonymizeReeccoinAmount") || reset)
        SoftSetArg("-anonymizereeccoinamount", settings.value("nAnonymizeReeccoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in reeccoin.cpp during shotdown to also remove the window positions
