...
#if defined(NDEBUG)
#error "REECCOIN cannot be compiled without assertions."
#endif
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zREEC mint output in tx %s",
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZREECModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (!UpdateZREECSupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zREEC supply", __func__);
        return DISCONNECT_FAILED;
    }
...
{
    util::ThreadRename("reeccoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    PublicCoinSpend publicSpend(params);
                    if (!ZREECModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
    ...
    if (!UpdateZREECSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zREEC supply for block=%s height=%d", __func__,
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateREECSupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zREEC serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Reeccoin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nReeccoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nReeccoinBadBlockBits)) {
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> reecInputs;
        std::vector<CTxIn> zREECInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zREECInputs.push_back(stakeIn);
            }else{
                reecInputs.push_back(stakeIn);
            }
        }
        const bool hasREECInputs = !reecInputs.empty();
        const bool hasZREECInputs = !zREECInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasREECInputs) {
                    ...
                    for (const CTxIn& reecIn : reecInputs)
                        if(reecIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            if(hasZREECInputs) {
                for (const CTxIn& zReecInput : zREECInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zReecInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zreec");
