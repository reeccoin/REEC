...
#include "zreec/zreecmodule.h"
#include "zreec/zreecwallet.h"
#include "zreec/zreectracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_REECCOIN = 61000, // inherited version..
...
enum ZerocoinSpendStatus {
    ZREEC_SPEND_OKAY = 0,                            // No error
    ZREEC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZREEC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZREEC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZREEC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZREEC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZREEC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZREEC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZREEC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZREEC_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZREEC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZREEC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZREEC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZREEC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZREEC_SPENT_USED_ZREEC = 14,                      // Coin has already been spend
    ZREEC_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZREEC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void doZReecRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZREEC);
    ...
    bool CreateZREECOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zPIV wallet
    std::unique_ptr<CzREECTracker> zreecTracker{nullptr};
    void setZWallet(CzREECWallet* zwallet);
    CzREECWallet* getZWallet();
