...
public:
    ...
    bool WriteZREECSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZREECSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZREECSeed_deprecated(uint256& seed);
    bool EraseZREECSeed();
    bool EraseZREECSeed_deprecated();

    bool WriteZREECCount(const uint32_t& nCount);
    bool ReadZREECCount(uint32_t& nCount);
