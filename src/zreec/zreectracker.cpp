...
#include "zreec/zreecwallet.h"


CzREECTracker::CzREECTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzREECTracker::~CzREECTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzREECTracker::Init()
{
...
bool CzREECTracker::Archive(CMintMeta& meta)
{
...
bool CzREECTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzREECTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzREECTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzREECTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzREECTracker::GetSerialHashes()
{
...
CAmount CzREECTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzREECTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzREECTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzREECTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzREECTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzREECTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzREECTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzREECTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzREECTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzREECTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzREECTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzREECWallet* zREECWallet)
{
    bool iszREECWalletInitialized = (nullptr != zREECWallet);
    CMintMeta meta;
    ...
    if (!iszREECWalletInitialized)
        zREECWallet = new CzREECWallet(wallet);
    meta.isSeedCorrect = zREECWallet->CheckSeed(dMint);
    if (!iszREECWalletInitialized)
        delete zREECWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzREECTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzREECTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzREECTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzREECTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzREECTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzREECTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzreec from DB\n", __func__, listDeterministicDB.size());
    }
