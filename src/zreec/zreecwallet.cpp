...
CzREECWallet::CzREECWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZREECSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZREECSeed_deprecated()) {
                    LogPrintf("%s: Updated zREEC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zreec seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zreec wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzREECWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZREECCount(nCountLastUsed);
    else if (!walletdb.ReadZREECCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzREECWallet::Lock()
{
    seedMaster.SetNull();
}

void CzREECWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzREECWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZREEC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzREECWallet::LoadMintPoolFromDB()
{
...
void CzREECWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzREECWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzREECWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zreecTracker->HasPubcoinHash(pMint.first)) {
...
bool CzREECWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZREEC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    wallet->zreecTracker->Add(dMint, true);
...
void CzREECWallet::SeedToZREEC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzREECWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzREECWallet::GenerateDeterministicZREEC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzREECWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzREECWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzREECWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
