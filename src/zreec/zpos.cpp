...
bool CLegacyZReecStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZReecStake from non zc-spend", __func__);
    ...
    *this = CLegacyZReecStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zreec stake origin", __func__);

    // All good
    return true;
}

CLegacyZReecStake::CLegacyZReecStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZReecStake::GetIndexFrom()
{
...
CAmount CLegacyZReecStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZReecStake::GetUniqueness() const
{
...
bool CLegacyZReecStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zREEC stake block: height %d outside range", __func__, nHeight);
