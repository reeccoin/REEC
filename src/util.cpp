...
const char * const REECCOIN_CONF_FILENAME = "reeccoin.conf";
const char * const REECCOIN_PID_FILENAME = "reeccoin.pid";
const char * const REECCOIN_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "REECCOIN";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Unix: ~/.reeccoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "REECCOINParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "REECCOINParams";
#else
    // Unix
    return pathRet / ".reeccoin-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", REECCOIN_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", REECCOIN_MASTERNODE_CONF_FILENAME));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty reeccoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override reeccoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", REECCOIN_PID_FILENAME));
