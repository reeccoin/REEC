...
bool CReecStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CReecStake from zerocoin spend", __func__);
...
bool CReecStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CReecStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CReecStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CReecStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CReecStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CReecStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CReecStake::GetUniqueness() const
{
...
CBlockIndex* CReecStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CReecStake::ContextCheck(int nHeight, uint32_t nTime)
{
