...
The build script we'll be using is contained in the REECCOIN github repository ([contrib/gitian-build.py](https://github.com/reeccoin/RECC/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the REECCOIN repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/reeccoin/REEC/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, reeccoin-detached-sigs, and reeccoin GitHub repos.
