...
*Setup instructions for a gitian build of REECCOIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the REECCOIN
...
to the REECCOIN GitHub release page.
...
- [Building REECCOIN Core](#building-reeccoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/reeccoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/reeccoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/reeccoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/reeccoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/reeccoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/reeccoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/reeccoin/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@reec.io) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/reeccoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/reeccoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/reeccoin-osx-*-build.assert
```
