...
class ZerocoinSpendTest(ReeccoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zreec_bal, reec_bal):
            zreec_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zreec_bal)
            reec_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], reec_bal)
            return zreec_bal, reec_bal
        ...
        zreec_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zreec_balance, 6666)
        ...
        zreec_balance, balance = check_balances(denom_2, zreec_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zreec_balance, balance = check_balances(denom_3, zreec_balance, balance)
        ...
        _, _ = check_balances(denom_1, zreec_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
