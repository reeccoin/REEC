...
class ReorgStakeTest(ReeccoinTestFramework):
    ...
    def check_money_supply(self, expected_reec, expected_zreec):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected REEC and zREEC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_reec))
            for denom in node['zREECsupply']:
                assert_equal(node['zREECsupply'][denom], DecimalAmt(expected_zreec[denom]))
    ...
    def run_test(self):
        ...
        # Check REEC and zREEC supply at the beginning
        # ------------------------------------------
        # zREEC supply: 2 coins for each denomination
        expected_zreec_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # REEC supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zreec_supply)
        ...
        # Verify that REEC and zREEC supplies were properly updated after the spends and reorgs
        self.log.info("Check REEC and zREEC supply...")
        ...
        expected_zreec_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zreec_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zreec_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zreec_supply)
