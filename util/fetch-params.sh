...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/REECCOINParams"
    else
        PARAMS_DIR="$HOME/.reeccoin-params"
    fi
fi
...
Failed to fetch the REECCOIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
REECCOIN - fetch-params.sh

This script will fetch the REECCOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common REECCOIN zkSNARK parameters. Note that it is
